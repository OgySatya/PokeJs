      ...
      <template v-for="(pokemon, index) in cards.value" :key="index">
        <button v-if="pokemon" class="w-20 h-24 lg:w-32 lg:h-40" @click="pick(pokemon.id, index)" :key="index">
          <div v-if="pokemon.flipped" :class="{ flip: pokemon.flipped }"
            class="w-20 h-24 lg:w-32 lg:h-40 bg-gradient-to-br from-primary via-secondary to-accent rounded-box border-2 border-base-300 lg:border-4 hover:border-neutral">
          </div>
          <div v-else
            :class="pokemon.types[0].type.name, { shake: pokemon.held_items.is_default }, { flip: !pokemon.flipped }"
            class="w-20 h-24 lg:w-32 lg:h-40 border-2 lg:border-8 rounded-box border-double border-neutral mx-auto">
        ...
        <div v-else>
          <div
            class="w-20 h-24 lg:w-32 lg:h-40 animate-pulse border-2 lg:border-8 rounded-box border-double border-neutral mx-auto flex items-center">
