...
<template>
  ...
  <div v-if="pickedPokemon" class="rounded-lg container m-5 w-max grid grid-cols-2 gap-4 mx-auto">
      ...
      <div v-if="types[1]":class="types[0].type.name" class="w-80 h-80">
        <div :class="types[1].type.name" style="border-radius: 50%;" class="w-80 h-80 " > 
        <img class="" :src="sprite"/>
        </div>
     </div>
     <div v-else :class="types[0].type.name" class="w-80 h-80">
        <img class="" :src="sprite"/>
     </div>
     <p class="text-2xl text-center uppercase font-bold my-1 text-teal-800">{{ pickedPokemon.name }}</p>
      ...
      <section class="mt-4 mx-auto">
          ...
          <span  class="grid grid-cols-3 gap-4">
            <div :class="dadu1.type,{shake: shake}" class="rounded-xl p-1.5 ">
              <img  class=" bg-white p-2 rounded-lg w-36 h-36" :src="dadu1.img" />
            </div>
            <div :class="dadu2.type,{shake: shake}" class="rounded-xl p-1.5 ">
              <img  class="w-36 h-36 bg-white p-2 rounded-lg" :src="dadu2.img" />
            </div>
            <div :class="dadu2.type,{shake: shake}" class="rounded-xl p-1.5 ">
              <img  class="w-36 h-36 bg-white p-2 rounded-lg" :src="dadu2.img" />
            </div>
